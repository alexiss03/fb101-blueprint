//
//  NYBPDeferredCompensationViewController.swift
//  Nymeria
//
...
class NYBPDeferredCompensationViewController: NYRPBaseViewController {

    override func viewDidLoad() {
        
        v = NYRPBaseView.fromNib("NYBPDeferredCompensationView")
        
        self.scrollView = v?.scrollView
        view = v
        
        formId = "NYBPDC"
        formName = "Bussiness Planning Deferred Compensation"
        ...
        pageName = "Deferred Compensation"
    ...
    override func addLookUpFields() {
        ...
        var vAPLookUp = self.getMortalityRateArr()
        ...
        for index in 4...124 {
            ...
            if index - 4  <= vAPLookUp.count {
                vAP.formula = "\(vAPLookUp[index-4])"
                vAP.text = "\(vAPLookUp[index-4])"
                vAP.value = "\(vAPLookUp[index-4])"
            } else {
                vAP.formula = "0"
                vAP.text = "0"
                vAP.value = "0"
            ...
            NYParser.shared.willAppendToFields(vAP)
        }
    }
    ...
    override func setUpFieldIds() {
        ...
        v?.r17TField3.isUserInteractionEnabled = true
        v?.r13TField2.isUserInteractionEnabled = false
        
        v!.r15Lbl2.preloadedVal =  "Future Value of Deferred Compensation @"
    ...
    override func setUpComputations() {
        // Z5
        self.v?.r4TField1.formula = "IF(OR(C9='';Z21='');'';IF(OR(Z20='';AB13<O13;AD10='');'';Z20*((1-1/E43^AD10)/E44*E43)))"
        
        // Z6
        self.v?.r5TField1.formula = "IF(OR(Z21='';AD10='');'';IF(AND(C9>0;AB13>=O13);L9*(1+J9)^MAX(AB13-O13;0)+R38;''))"
        
        // Z7
        self.v?.r6TField1.formula = "IF(OR(C9='';Z21='');'';IF(OR(Z5='';Z6='');'';MAX(Z5-Z6;0)))"
        
        //AC10
        self.v?.r9Lbl4.formula = "IF(OR($Main.I39='';AB13='';$v.AO1='');'';INDEX($v.AP4:$v.AP124,AB13+1))"
        
        // O13
        self.v?.r10TField3.formula = "IF(C9>0;ROUNDDOWN((TODAY-C9)/365;0);'')"
        
        // AF12
        self.v?.r10Lbl5.formula = "IF(OR(AB13='';$v.AO1='');'';'Life Expectancy @ '&TEXT(INDEX($v.AP4:$v.AP124;AB13+1)+AB13;'0'))"
        ...
        // L20
        self.v?.r16TField1.formula = "IF(OR(C9='';Z21='');'';IF(OR(Z20=0;AB13<O13);'';Z20/(1+X21)^(AB13-O13)))"
        ...
        //Z19
        self.v?.r15Lbl2.formula = "'Future Retirement Pension Required @'&AB13"
